    ...
    addNewLineCondition (from, to, oldTo) {
      ...
      if (oldTo) {
        ...
        try {
                ...
                if (conn && conn.source && conn.target) {
                  try {
                    ...
                    if (!connError.message?.includes("Cannot read properties of null")) {
                      console.error('Error deleting specific connection:', connError)
                    }
                  }
                }
              })
          ...
          console.error('Error getting specific connections:', error)
        }
      }
          ...
          if (conn && conn.source && conn.target) {
            ...
            setTimeout(() => {
              try {
                ...
                if (!connError.message?.includes("Cannot read properties of null")) {
                  console.error('Error deleting connection:', connError)
                }
              }
            }, 0)
          }
        } catch (error) {
          console.error('Error getting connection for deletion:', error)
        }
      }
    },
    ...
    deleteNode (node) {
      this.$q.dialog({
        ...
        try {
          ...
          console.error('Error getting connections for node deletion:', error)
        }
