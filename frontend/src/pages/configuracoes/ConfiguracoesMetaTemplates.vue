<template>
  <div v-if="userProfile === 'admin'">
    <q-tabs
      ...
      v-if="!showOnlyPhones"
    >
      <q-tab name="waba" :label="$t('configuracaoMetaPanel.tabs.phones') || 'WABA'" />
      <q-tab name="facebook" :label="$t('configuracaoMetaPanel.templates.facebook')" />
    </q-tabs>

    <q-separator />

    <q-tab-panels v-model="tab" animated>
      <!-- Aba WABA - Templates e Criar Templates -->
      <q-tab-panel name="waba" class="q-pa-none" v-if="!showOnlyPhones">
        <q-tabs
          v-model="wabaSubTab"
          dense
          class="text-grey"
        ...
        <q-separator />
        <q-tab-panels v-model="wabaSubTab" animated>
          <!-- Sub-aba de Listagem de Templates WABA -->
          ...
          <!-- Sub-aba de Criação de Templates WABA -->
      ...
      <!-- Aba Facebook - Templates e Criar Templates -->
      <q-tab-panel name="facebook" class="q-pa-none" v-if="!showOnlyPhones">
        <q-tabs
        ...
        <q-separator />
        <q-tab-panels v-model="facebookSubTab" animated>
          <!-- Sub-aba de Listagem de Templates Facebook -->
          <q-tab-panel name="templates" class="q-pa-none">
            <div class="q-pa-md">
              ...
              <div class="row q-mb-md">
                <div class="col-12 col-md-6" style="margin-right: 10px;">
                  <q-select
                    ...
                    :label="$t('configuracaoMetaPanel.templates.selectMessengerConnection') || 'Selecionar Conexão Messenger'"
                    outlined
                    dense
                    option-label="label"
                    option-value="value"
                    @update:model-value="listarTemplatesMessenger"
                    :loading="loadingTemplatesMessenger"
                  >
                    <template v-slot:no-option>
                      <q-item>
                        <q-item-section class="text-grey">
                          {{ $t('configuracaoMetaPanel.templates.noMessengerConnectionsAvailable') || 'Nenhuma conexão Messenger disponível' }}
                        </q-item-section>
                      </q-item>
                    </template>
                  </q-select>
                ...
                <div class="col-12 col-md-5">
                  <q-btn
                    ...
                    color="primary"
                    icon="refresh"
                    @click="listarTemplatesMessenger"
                    :loading="loadingTemplatesMessenger"
              ...
              <div v-if="cMessengerSessions.length === 0" class="row q-mb-md">
                <div class="col-12">
                  <q-banner class="bg-orange-1 text-orange-8">
                    ...
                    {{ $t('configuracaoMetaPanel.templates.noMessengerConnections') || 'Nenhuma conexão Messenger encontrada. Configure uma conexão Messenger primeiro na seção de Canais de Comunicação.' }}
                  </q-banner>
                </div>
              </div>

              <!-- Lista de Templates Messenger -->
              <div v-if="templatesMessenger.length > 0" class="row q-col-gutter-md">
                <div 
                  v-for="template in templatesMessenger" 
                  :key="template.notification_messages_token || template.id"
                  class="col-12 col-md-6 col-lg-4"
                >
                  <q-card>
                    <q-card-section>
                      <div class="text-h6">{{ template.topic_title || 'Template sem título' }}</div>
                      <div class="text-caption text-grey">
                        Token: {{ template.notification_messages_token?.substring(0, 20) }}...
                      </div>
                      <div class="text-caption text-grey q-mt-sm">
                        Status: {{ template.user_token_status || 'N/A' }}
                      </div>
                      <div class="text-caption text-grey">
                        Próximo envio: {{ template.next_eligible_time ? new Date(template.next_eligible_time * 1000).toLocaleString() : 'N/A' }}
                      </div>
                    </q-card-section>
                    <q-card-actions align="right">
                      <q-btn 
                        ...
                        dense 
                        color="primary" 
                        :label="$t('configuracaoMetaPanel.templates.use') || 'Usar'"
                        @click="usarTemplateMessenger(template)"
                      />
                    </q-card-actions>
                  </q-card>
                </div>
              ...
              <div v-if="templatesMessenger.length === 0 && selectedMessenger && !loadingTemplatesMessenger" class="row q-mb-md">
                <div class="col-12">
                  <q-banner class="bg-blue-1 text-blue-8">
                    <template v-slot:avatar>
                      <q-icon name="info" color="blue" />
                    </template>
                    {{ $t('configuracaoMetaPanel.templates.noTemplatesFound') || 'Nenhum template encontrado para esta conexão.' }}
                  </q-banner>
                </div>
              </div>
            </div>
          ...
          <!-- Sub-aba de Criação de Templates Facebook -->
          <q-tab-panel name="create" class="q-pa-none">
            <div class="q-pa-md" style="min-height: 800px !important;">
              <!-- Mensagem informando que a criação de templates Facebook será liberada nas próximas versões -->
              <div class="row q-mb-md">
                <div class="col-12">
                  <q-banner class="bg-blue-1 text-blue-8">
                    <template v-slot:avatar>
                    ...
                    {{ $t('configuracaoMetaPanel.templates.facebookCreateComingSoon') || 'A criação de templates Facebook será liberada nas próximas versões.' }}
                  </q-banner>
                </div>
              </div>

              <!-- Seletor de Conexão Messenger - Desabilitado temporariamente -->
              <div class="row q-mb-md" style="padding-bottom: 10px;">
                <div class="col-12 col-md-6" style="margin-right: 10px;">
                  <q-select
                    v-model="selectedMessenger"
                    ...
                    option-label="label"
                    option-value="value"
                    :loading="loadingCreateMessenger"
                    disable
                  >
                    <template v-slot:no-option>
                      <q-item>
              ...
              <!-- Formulário desabilitado - não será exibido até que a funcionalidade seja liberada -->
              <div v-if="false">
                <div class="row q-col-gutter-lg" style="align-items: flex-start;">
                  <!-- Coluna do Formulário -->
                  <div class="col-12 col-md-6">
                    <q-form @submit="criarTemplateMessenger()" class="q-gutter-md">
                      <!-- Informações Básicas -->
                      <q-card class="q-mb-md">
                        <q-card-section>
                          <h6 class="text-weight-bold">
                            {{ $t('configuracaoMetaPanel.templates.createForm.basicInfo') }}
                          </h6>
                          <p class="text-caption text-grey-6 q-mt-sm">
                            {{ $t('configuracaoMetaPanel.templates.createForm.requiredFieldsNote') }}
                          </p>
                        ...
                        <q-card-section>
                          <div class="row q-col-gutter-md">
                            <div class="col-12 col-md-6">
                              <q-input
                                v-model="novoTemplateMessenger.name"
                                :label="$t('configuracaoMetaPanel.templates.createForm.name') + ' *'"
                                outlined
                                dense
                                :rules="[val => !!val || $t('configuracaoMetaPanel.templates.createForm.nameRequired')]"
                            ...
                            <div class="col-12 col-md-6">
                              <q-select
                                v-model="novoTemplateMessenger.language"
                                :options="languageOptions"
                                :label="$t('configuracaoMetaPanel.templates.createForm.language') + ' *'"
                                outlined
                                dense
                            ...
                            <div class="col-12">
                              <q-input
                                :model-value="messengerCategoryLabel"
                                :label="$t('configuracaoMetaPanel.templates.createForm.category')"
                                outlined
                                dense
                                readonly
                                :hint="$t('configuracaoMetaPanel.templates.messengerCategoryHint') || 'Messenger só suporta templates UTILITY'"
                                persistent-hint
                              />
                            </div>
                          </div>
                      ...
                      <q-card class="q-mb-md">
                        <q-card-section>
                          <div class="row items-center justify-between">
                            ...
                            <div class="row items-center q-gutter-sm">
                              <q-select
                                v-model="novoTipoComponenteMessenger"
                                :options="componentTypeOptionsMessenger"
                                :label="$t('configuracaoMetaPanel.common.type')"
                                outlined
                                dense
                                style="min-width: 150px;"
                              ...
                              <q-btn
                                ...
                                color="primary"
                                icon="add"
                                @click="adicionarComponenteMessenger"
                                :disable="!novoTipoComponenteMessenger"
                              />
                            </div>
                          </div>
                        </q-card-section>
                        <q-separator />
                        <q-card-section>
                          <div v-for="(componente, index) in novoTemplateMessenger.components" :key="`componente-messenger-${index}-${componente.type}`" class="q-mb-md">
                            <q-card outlined>
                              <q-card-section>
                                <div class="row items-center justify-between q-mb-sm">
                                  <h6 class="text-weight-medium">{{ $t('configuracaoMetaPanel.templates.createForm.component') }} {{ index + 1 }}</h6>
                                  <q-btn
                                    ...
                                    color="negative"
                                    icon="delete"
                                    @click="removerComponenteMessenger(index)"
                                  />
                                </div>
                                ...
                                <div class="row q-col-gutter-md">
                                  <div class="col-12 col-md-6">
                                    <q-select
                                      ...
                                      emit-value
                                      map-options
                                      @update:model-value="onComponentTypeChangeMessenger(componente)"
                                    />
                                  </div>
                                  <div class="col-12 col-md-6" v-if="componente.type === 'HEADER'">
                                    <q-select
                                      ...
                                      emit-value
                                      map-options
                                      @update:model-value="onHeaderFormatChangeMessenger(componente)"
                                    />
                                  </div>
                                </div>
                                ...
                                <!-- Campos específicos por tipo de componente -->
                                <div class="row q-col-gutter-md q-mt-md">
                                  <!-- Campo de texto para HEADER e BODY (FOOTER não é suportado pelo Messenger) -->
                                  <div class="col-12" v-if="componente.type === 'HEADER' || componente.type === 'BODY'">
                                    <q-input
                                      v-model="componente.text"
                                      :label="$t('configuracaoMetaPanel.templates.createForm.text')"
                                      outlined
                                ...
                                <div v-if="componente.type === 'BUTTONS'" class="q-mt-md">
                                  <div class="row items-center justify-between q-mb-sm">
                                    ...
                                    <q-btn
                                      ...
                                      icon="add"
                                      size="sm"
                                      @click="adicionarBotaoMessenger(componente)"
                                    />
                                  </div>
                                      ...
                                      <q-card-section>
                                        <div class="row items-center justify-between q-mb-sm">
                                          ...
                                          <q-btn
                                            ...
                                            icon="delete"
                                            size="sm"
                                            @click="removerBotaoMessenger(componente, botaoIndex)"
                                          />
                                        </div>
                                        ...
                                        <div class="row q-col-gutter-md">
                                          <div class="col-12 col-md-6">
                                            <q-select
                                              v-model="botao.type"
                                              :options="messengerButtonTypeOptions"
                                              :label="$t('configuracaoMetaPanel.templates.createForm.buttonType')"
                                              outlined
                                              dense
                                              option-value="value"
                                              ...
                                              emit-value
                                              map-options
                                              @update:model-value="onButtonTypeChangeMessenger(botao)"
                                            />
                                          </div>
                                          <div class="col-12 col-md-6">
                                            <q-input
                                        ...
                                        <div class="row q-col-gutter-md q-mt-md" v-if="botao.type === 'POSTBACK'">
                                          <div class="col-12">
                                            <q-input
                                              v-model="botao.payload"
                                              :label="$t('configuracaoMetaPanel.templates.createForm.payload') || 'Payload'"
                                              outlined
                                              dense
                                            />
                                          </div>
                      ...
                      <div class="row justify-end q-gutter-md">
                        <q-btn
                          :label="$t('common.cancel')"
                          color="warning"
                          @click="limparFormularioMessenger"
                        />
                        <q-btn
                          :label="$t('configuracaoMetaPanel.templates.createForm.submit')"
                          color="primary"
                          type="submit"
                          :loading="loadingCreateMessenger"
                        />
                  ...
                  <!-- Coluna do Preview -->
                  <div class="col-12 col-md-6">
                    <div class="floating-preview-column" ref="previewColumnMessenger">
                      <div class="text-subtitle1 q-mb-sm text-center">{{ $t('configuracaoMetaPanel.preview.title') }}</div>
                      <div class="mobile-mockup q-mx-auto">
                        <div class="mobile-notch"></div>
                        <div class="mobile-screen">
                          <div
                            ...
                            :class="{ 'bubble-max-width': shouldMaximizeBubbleWidth }"
                          >
                            <div class="preview-header" v-if="getPreviewHeaderTypeMessenger && getPreviewHeaderTypeMessenger()">
                              <q-icon
                                v-if="getPreviewHeaderTypeMessenger() === 'IMAGE' && (!getPreviewHeaderMediaUrlMessenger || !getPreviewHeaderMediaUrlMessenger())"
                                name="image" size="xl" color="grey-7" class="preview-header-icon"
                              />
                              <q-img
                                v-if="getPreviewHeaderTypeMessenger() === 'IMAGE' && getPreviewHeaderMediaUrlMessenger && getPreviewHeaderMediaUrlMessenger()"
                                :src="getPreviewHeaderMediaUrlMessenger()" spinner-color="primary" contain
                                style="max-height: 160px;" class="preview-header-image"
                              >
                                <template v-slot:error>
                                  <div class="absolute-full flex flex-center bg-negative text-white text-caption q-pa-xs">
                              ...
                              <q-icon
                                v-if="getPreviewHeaderTypeMessenger() === 'VIDEO'"
                                name="movie" size="xl" color="grey-7" class="preview-header-icon"
                              />
                              <q-icon
                                v-if="getPreviewHeaderTypeMessenger() === 'DOCUMENT'"
                                name="description" size="xl" color="grey-7" class="preview-header-icon"
                              />
                              <div v-if="getPreviewHeaderTypeMessenger() === 'TEXT'" v-html="previewHeaderContentMessenger" class="preview-header-text"></div>
                              <div v-if="getPreviewHeaderTypeMessenger() === 'VIDEO' || getPreviewHeaderTypeMessenger() === 'DOCUMENT'" class="preview-header-caption">
                                {{ (getPreviewHeaderMediaUrlMessenger && getPreviewHeaderMediaUrlMessenger()) || $t('configuracaoMetaPanel.preview.urlNotAvailable') }}
                              </div>
                            </div>

                            <div class="preview-body" v-if="previewBodyContentMessenger" v-html="previewBodyContentMessenger"></div>

                            <!-- FOOTER removido: Messenger não suporta FOOTER -->

                            <div class="preview-buttons" v-if="previewButtonTypeMessenger !== 'NONE'">
                              <div v-if="previewButtonTypeMessenger === 'QUICK_REPLY'" class="preview-quick-reply-buttons">
                                <q-chip dense v-for="(button, index) in previewQuickReplyButtonsMessenger" :key="`prev-qr-messenger-${index}`" clickable color="blue-1" text-color="blue-8" icon="reply" size="sm" class="q-ma-xs">
                                  {{ button.text || $t('configuracaoMetaPanel.dynamic.buttonNumber', { index: index + 1 }) }}
                                </q-chip>
                              </div>
                              <div v-if="previewButtonTypeMessenger === 'CALL_TO_ACTION'" class="preview-action-buttons">
                                <q-btn v-for="(button, index) in previewActionButtonsMessenger" :key="`prev-cta-messenger-${index}`" :icon="button.type === 'PHONE_NUMBER' ? 'call' : 'link'" :label="button.text || $t('configuracaoMetaPanel.dynamic.actionButton', { index: index + 1 })" no-caps flat dense size="sm" class="full-width q-my-xs preview-cta-button" align="center" />
                              </div>
                            </div>
                          </div>
                        </div>
      ...
      <!-- Aba de Telefones - Apenas quando showOnlyPhones for true -->
      <q-tab-panel name="phones" class="q-pa-none" v-if="showOnlyPhones">
        <div class="q-pa-md">
          <!-- Seletor de Conexão WABA -->
          <div class="row q-mb-md" style="padding-bottom: 10px;">
    ...
    <!-- Modal de Configuração de Webhook COEX -->
    <q-dialog v-model="showWebhookCoexModal" persistent>
      <q-card style="min-width: 500px">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">
            Webhook COEX
        ...
        <q-card-section>
          <div class="q-mb-md">
            <h6 class="text-weight-bold">Configurar Webhook COEX</h6>
            <p class="text-grey-7">
              Configure o URL de retorno de chamada alternativo para o número de telefone comercial (COEX).
            </p>
          </div>

          <div class="q-gutter-md">
            <q-input
              v-model="webhookCoexData.callbackUrl"
              ...
              :hint="'URL Callback (ex: ' + cBaseUrlIntegração + '/1)'"
              persistent-hint
              :rules="[val => !!val || 'URL de callback é obrigatória']"
            />
            
            <q-input
              v-model="webhookCoexData.verifyToken"
              label="Token de Verificação"
              outlined
              dense
              placeholder="Token de verificação do webhook"
              hint="Token usado para verificar o webhook"
              persistent-hint
              :rules="[val => !!val || 'Token de verificação é obrigatório']"
            />
          </div>
          ...
          <div class="row justify-end q-gutter-md q-mt-md">
            ...
            />
            <q-btn
              label="Configurar Webhook COEX"
              color="primary"
              @click="sobrescreverWebhookCoex"
              :loading="loadingWebhookCoex"
              icon="settings_phone"
            />
          </div>
        </q-card-section>
...
import { 
  BuscarTemplatesMessenger,
  EnviarTemplateMarketingMessenger,
  EnviarTemplateUtilityMessenger,
  CriarTemplateMarketingMessenger,
  CriarTemplateUtilityMessenger
} from 'src/service/messenger.js'
...
export default defineComponent({
  ...
  data() {
    return {
      usuario,
      tab: this.showOnlyPhones ? 'phones' : (this.initialTab || 'waba'),
      wabaSubTab: 'templates',
      facebookSubTab: 'templates',
      userProfile: 'user',
      ...
      // Messenger/Facebook templates
      selectedMessenger: null,
      loadingTemplatesMessenger: false,
      templatesMessenger: [],
      loadingCreateMessenger: false,
      novoTemplateMessenger: {
        name: '',
        language: '',
        category: 'UTILITY', // Messenger só suporta UTILITY
        components: []
      },
      novoTipoComponenteMessenger: null,
      // Messenger só aceita templates UTILITY (MARKETING não é suportado via message_templates endpoint)
      messengerCategoryOptions: [
        { label: this.$t('configuracaoMetaPanel.label.utility'), value: 'UTILITY' }
      ],
      messengerButtonTypeOptions: [
      ...
      previewHeaderContentMessenger: '',
      previewBodyContentMessenger: '',
      previewFooterContentMessenger: '',
      previewButtonTypeMessenger: 'NONE',
      previewQuickReplyButtonsMessenger: [],
      previewActionButtonsMessenger: [],
      filtros: {
        status: null,
        category: null,
        language: null,
      ...
      // Array base de idiomas (será ordenado alfabeticamente via computed)
      languageOptionsBase: [
        { label: this.$t('configuracaoMetaPanel.label.ptBR'), value: 'pt_BR' },
        { label: this.$t('configuracaoMetaPanel.label.enUS'), value: 'en_US' },
        { label: this.$t('configuracaoMetaPanel.label.esES'), value: 'es_ES' },
      ...
      // Opções de componentes para Messenger (sem FOOTER, pois a API não aceita)
      componentTypeOptionsMessenger: [
        { label: this.$t('configuracaoMetaPanel.label.header'), value: 'HEADER' },
        { label: this.$t('configuracaoMetaPanel.label.body'), value: 'BODY' },
        { label: this.$t('configuracaoMetaPanel.label.buttons'), value: 'BUTTONS' }
      ],
      ...
      showWebhookCoexModal: false,
      webhookCoexData: {
        callbackUrl: '',
        verifyToken: ''
      },
      loadingWebhookCoex: false,
  ...
  computed: {
    ...
    cSessions() {
      return this.whatsapps.filter(w => w.type === 'waba' && !w.isDeleted);
    },
    cSessionsOptions() {
    ...
    // Opções de idiomas ordenadas alfabeticamente
    languageOptions() {
      return [...this.languageOptionsBase].sort((a, b) => {
    ...
    // Label da categoria UTILITY para Messenger
    messengerCategoryLabel() {
      return this.messengerCategoryOptions[0]?.label || this.$t('configuracaoMetaPanel.label.utility') || 'UTILITY';
    },
  },
  watch: {
  ...
  methods: {
    ...
    async listarTemplates() {
      try {
        ...
        const templatesOrdenados = (response.data.data || []).sort((a, b) => {
    ...
    async listarTemplatesMessenger() {
      try {
        this.loadingTemplatesMessenger = true
        
        // Verificar se há conexões Messenger disponíveis
        if (this.cMessengerSessions.length === 0) {
          this.$q.notify({
            type: 'negative',
            message: this.$t('configuracaoMetaPanel.notifications.noMessengerConnectionsFound') || 'Nenhuma conexão Messenger encontrada',
            timeout: 5000
          })
          return
        }

        // Se não há Messenger selecionado, usar o primeiro disponível
        if (!this.selectedMessenger && this.cMessengerSessionsOptions.length > 0) {
        ...
        if (!this.selectedMessenger) {
          this.$q.notify({
            type: 'negative',
            message: this.$t('configuracaoMetaPanel.notifications.selectMessengerToList') || 'Selecione uma conexão Messenger para listar templates',
            timeout: 3000
          })
          return
        }

        const response = await BuscarTemplatesMessenger(this.selectedMessenger.value)
        this.templatesMessenger = response.data.data || []
      } catch (error) {
        console.error('Erro ao listar templates Messenger:', error)
        this.$q.notify({
          type: 'negative',
          message: this.$t('configuracaoMetaPanel.templates.errors.listError') || 'Erro ao listar templates',
          timeout: 3000
        })
      } finally {
        this.loadingTemplatesMessenger = false
      }
    },

    usarTemplateMessenger(template) {
      // Implementar lógica para usar template Messenger
      this.$q.notify({
        type: 'info',
        message: this.$t('configuracaoMetaPanel.templates.templateSelected') || 'Template selecionado',
        timeout: 2000
      })
      // Aqui você pode adicionar lógica para usar o template, como abrir um modal ou navegar para outra página
    },

    async criarTemplateMessenger() {
      try {
        this.loadingCreateMessenger = true
        
        if (!this.selectedMessenger) {
          this.$q.notify({
            type: 'negative',
            message: this.$t('configuracaoMetaPanel.notifications.selectMessengerToCreate') || 'Selecione uma conexão Messenger para criar o template',
            timeout: 5000
          })
          return
        }

        if (!this.novoTemplateMessenger.name || !this.novoTemplateMessenger.name.trim()) {
          this.$q.notify({
            type: 'negative',
        ...
        if (!this.novoTemplateMessenger.language) {
          this.$q.notify({
            type: 'negative',
        ...
        // Categoria sempre será UTILITY para Messenger (não precisa validar)

        // Filtrar componentes FOOTER, pois a API do Messenger não aceita esse tipo
        const validComponents = this.novoTemplateMessenger.components.filter(componente => {
          const type = componente.type?.toUpperCase();
          return type === 'HEADER' || type === 'BODY' || type === 'BUTTONS';
        });

        if (validComponents.length === 0) {
          this.$q.notify({
            type: 'negative',
            message: this.$t('configuracaoMetaPanel.templates.errors.noValidComponents') || 'Adicione pelo menos um componente válido (HEADER, BODY ou BUTTONS)',
            timeout: 5000
          })
          this.loadingCreateMessenger = false
          return
        }

        // Garantir que a categoria seja UTILITY (única opção suportada pelo Messenger)
        const category = 'UTILITY';
        
        const templateData = {
          whatsappId: this.selectedMessenger.value,
          pageId: this.selectedMessenger.fbPageId,
          template: {
            name: this.novoTemplateMessenger.name,
            language: this.novoTemplateMessenger.language?.value || this.novoTemplateMessenger.language,
            category: category,
            components: validComponents
          }
        }

        // Messenger só suporta templates UTILITY
        const response = await CriarTemplateUtilityMessenger(templateData)
        ...
        this.limparFormularioMessenger()
        this.facebookSubTab = 'templates'
        await this.listarTemplatesMessenger()
      } catch (error) {
        console.error('Erro ao criar template Messenger:', error)
        
        this.$q.notify({
          type: 'negative',
          message: error.response?.data?.error || this.$t('configuracaoMetaPanel.notifications.createError'),
          timeout: 5000
        })
      } finally {
        this.loadingCreateMessenger = false
    ...
    adicionarComponenteMessenger() {
      const novoComponente = {
        type: this.novoTipoComponenteMessenger,
        text: ''
      }
      ...
      this.novoTemplateMessenger.components.push(novoComponente)
      ...
      this.novoTipoComponenteMessenger = null
      this.atualizarPreviewMessenger()
    },

    removerComponenteMessenger(index) {
      this.novoTemplateMessenger.components.splice(index, 1)
      this.atualizarPreviewMessenger()
    },

    adicionarBotaoMessenger(componente) {
    ...
    removerBotaoMessenger(componente, index) {
      componente.buttons.splice(index, 1)
      this.atualizarPreviewMessenger()
    },

    onComponentTypeChangeMessenger(componente) {
    ...
    onButtonTypeChangeMessenger(botao) {
      if (botao.type === 'URL') {
        botao.url = ''
      }
      ...
      if (botao.type === 'POSTBACK') {
        botao.payload = ''
      }
      this.atualizarPreviewMessenger()
    },

    onHeaderFormatChangeMessenger(componente) {
    ...
    limparFormularioMessenger() {
      this.novoTemplateMessenger = {
        name: '',
        language: '',
        category: 'UTILITY', // Messenger só suporta UTILITY
        components: []
    ...
    atualizarPreviewMessenger() {
      this.previewHeaderContentMessenger = ''
      this.previewBodyContentMessenger = ''
      this.previewFooterContentMessenger = ''
      this.previewButtonTypeMessenger = 'NONE'
      this.previewQuickReplyButtonsMessenger = []
      this.previewActionButtonsMessenger = []

      // Categoria sempre será UTILITY para Messenger
      if (!this.novoTemplateMessenger.name || !this.novoTemplateMessenger.language) {
        return
      }

      this.novoTemplateMessenger.components.forEach(componente => {
        if (componente.type === 'HEADER') {
          if (componente.format === 'TEXT' && componente.text) {
            this.previewHeaderContentMessenger = componente.text
          }
        } else if (componente.type === 'BODY') {
          this.previewBodyContentMessenger = componente.text || ''
        }
        // FOOTER removido: Messenger não suporta FOOTER
        else if (componente.type === 'BUTTONS') {
          if (componente.buttons && componente.buttons.length > 0) {
            this.previewButtonTypeMessenger = 'CALL_TO_ACTION'
            this.previewActionButtonsMessenger = componente.buttons.map(b => ({
    ...
    getPreviewHeaderTypeMessenger() {
      const headerComponent = this.novoTemplateMessenger.components?.find(c => c.type === 'HEADER')
      if (headerComponent && headerComponent.format) {
        return headerComponent.format
      }
      return null
    ...
    getPreviewHeaderMediaUrlMessenger() {
      const headerComponent = this.novoTemplateMessenger.components?.find(c => c.type === 'HEADER')
      if (headerComponent && headerComponent.example) {
        if (typeof headerComponent.example === 'string') {
          return headerComponent.example
    ...
    async abrirModalWebhook() {
      ...
      try {
        ...
        console.error('Erro ao buscar metaToken:', error)
        this.$q.notify({
          type: 'negative',
    ...
    async sobrescreverWebhook() {
      try {
        ...
        console.error('Erro ao sobrescrever webhook:', error)
        console.error('Detalhes do erro:', {
          message: error.message,
          response: error.response?.data,
          status: error.response?.status
    ...
    // Método para abrir modal de webhook COEX
    async abrirModalWebhookCoex() {
      if (!this.selectedWhatsapp) {
        this.$q.notify({
          type: 'negative',
          message: this.$t('configuracaoMetaPanel.notifications.selectWabaFirst'),
      ...
      try {
        ...
        this.webhookCoexData = {
          callbackUrl: this.cBaseUrlIntegração + '/' + this.usuario.tenantId,
          verifyToken: metaToken
        }
        
        this.showWebhookCoexModal = true
    ...
    // Método para sobrescrever webhook COEX
    async sobrescreverWebhookCoex() {
      try {
        if (!this.webhookCoexData.callbackUrl || !this.webhookCoexData.verifyToken) {
          this.$q.notify({
            type: 'negative',
            message: 'Preencha todos os campos obrigatórios',
            timeout: 3000
        ...
        if (!this.selectedWhatsapp.tokenAPI) {
          this.$q.notify({
            type: 'negative',
            message: 'ID do número de telefone não encontrado',
            timeout: 3000
          })
          return
        }

        this.loadingWebhookCoex = true
        
        const requestData = {
          businessPhoneNumberId: this.selectedWhatsapp.tokenAPI,
          wabaVersion: this.selectedWhatsapp.wabaVersion || 'v24.0',
          wabaToken: this.selectedWhatsapp.bmToken,
          overrideCallbackUri: this.webhookCoexData.callbackUrl,
          verifyToken: this.webhookCoexData.verifyToken
        }
        
        const response = await OverrideCallbackCoexUrl(requestData)
        
        this.$q.notify({
          type: 'positive',
          message: 'Webhook COEX configurado com sucesso!',
          timeout: 5000
        })
        
        this.showWebhookCoexModal = false
        
      } catch (error) {
        console.error('Erro ao sobrescrever webhook COEX:', error)
        console.error('Detalhes do erro:', {
          message: error.message,
          response: error.response?.data,
          status: error.response?.status
        ...
        this.$q.notify({
          type: 'negative',
          message: error.response?.data?.error || 'Erro ao configurar webhook COEX. Verifique os dados e tente novamente.',
          timeout: 5000
        })
      } finally {
        this.loadingWebhookCoex = false
      }
    },
    ...
    // Método para verificar conta Instagram
  },
