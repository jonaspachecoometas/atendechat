  ...
  <q-list separator
    ...
    <!-- :clickable="ticket.status !== 'pending' && (ticket.id !== $store.getters['ticketFocado'].id || $route.name !== 'chat')" -->
      ...
      :class="{
        ...
        'ticket-selecao-ativa': modoSelecao
      }">
      <q-item-section avatar
        class="q-px-none">
        <div v-if="modoSelecao" @click.stop class="checkbox-wrapper">
          <q-checkbox
            v-model="selecionado"
...
#item-ticket-houve:hover:not(.ticket-selecao-ativa)
  background: $blue-1
  transition: all .2s

.ticket-selecao-ativa
...
.ticket-selecao-ativa .checkbox-wrapper
  pointer-events: auto
  cursor: pointer
