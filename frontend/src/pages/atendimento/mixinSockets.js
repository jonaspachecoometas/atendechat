...
function cleanupSocketTimers() {
  ...
  socketTimers = []
  // Limpa também os timers de debounce
  cleanupDebounceTimers()
}

// Sistema de debounce para agrupar múltiplas chamadas de ConsultarTickets
let consultarTicketsDebounceTimer = null
let consultarTicketsInProgress = false
let pendingCallbacks = []
const DEBOUNCE_DELAY = 500 // 500ms de delay para agrupar chamadas (mesmo valor do socketInitial)

function debouncedConsultarTickets(params, callback) {
  ...
  if (consultarTicketsInProgress) {
    return
  }

  // Limpa o timer anterior se existir
  if (consultarTicketsDebounceTimer) {
    clearTimeout(consultarTicketsDebounceTimer)
  }

  // Cria novo timer para agrupar chamadas próximas
  consultarTicketsDebounceTimer = setTimeout(async () => {
    consultarTicketsInProgress = true
    ...
    try {
      ...
      pendingCallbacks.forEach(cb => {
        try {
          ...
          console.error('Erro ao executar callback de ConsultarTickets (mixinSockets):', err)
        }
      })
    } catch (err) {
      // Silenciar erros de timeout - não mostrar no console
      if (err?.code !== 'ECONNABORTED' && !err?.message?.includes('timeout')) {
        console.log('error debouncedConsultarTickets (mixinSockets)', err)
      }
      ...
      pendingCallbacks.forEach(cb => {
        try {
          ...
          console.error('Erro ao executar callback de ConsultarTickets (erro) (mixinSockets):', e)
        }
      })
    } finally {
      // Limpa estado
  ...
  addSocketTimer(consultarTicketsDebounceTimer)
}
