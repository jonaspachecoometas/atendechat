...
export default {
  ...
  methods: {
    ...
    async consultarTicketsOtimizado(params, options = {}) {
      ...
      // Normalizar parâmetros para garantir comparação correta do cache
      // Incluir explicitamente showAll e includeClosed para evitar cache incorreto
      const normalizedParams = {
        pageNumber: params.pageNumber || 1,
        status: Array.isArray(params.status) ? params.status.slice().sort() : params.status,
        searchParam: params.searchParam || '',
        queuesIds: Array.isArray(params.queuesIds) ? params.queuesIds.slice().sort() : params.queuesIds,
        whatsappIds: Array.isArray(params.whatsappIds) ? params.whatsappIds.slice().sort() : params.whatsappIds,
        selectedUser: Array.isArray(params.selectedUser) ? params.selectedUser.slice().sort() : params.selectedUser,
        withUnreadMessages: params.withUnreadMessages || false,
        isNotAssignedUser: params.isNotAssignedUser || false,
        includeNotQueueDefined: params.includeNotQueueDefined !== undefined ? params.includeNotQueueDefined : true,
        showAll: params.showAll || false,
        includeClosed: params.includeClosed || false, // Importante: sempre incluir explicitamente
      ...
      const promise = new Promise((resolve, reject) => {
        this.ticketsQueryDebounce = setTimeout(async () => {
          try {
            // Passar forceRefresh para ConsultarTickets do service
            const response = await ConsultarTickets(params, { forceRefresh });
            
            // Atualizar cache - usar parâmetros normalizados para garantir comparação correta
