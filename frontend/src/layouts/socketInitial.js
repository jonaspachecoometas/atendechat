...
export default {
  data() {
    return {
      ...
      timers: [],
      // Sistema de debounce para ConsultarTickets
      consultarTicketsDebounceTimer: null,
      consultarTicketsInProgress: false,
      pendingCallbacks: [],
    }
  },
  methods: {
    ...
    cleanupMemory() {
      ...
      if (this.consultarTicketsDebounceTimer) {
        clearTimeout(this.consultarTicketsDebounceTimer);
        this.consultarTicketsDebounceTimer = null;
      }
      // Reseta estado do debounce
      this.consultarTicketsInProgress = false;
    ...
    socketInitial () {
      ...
      // Sistema de debounce para agrupar múltiplas chamadas de ConsultarTickets
      const DEBOUNCE_DELAY = 500; // 500ms de delay para agrupar chamadas

      const debouncedConsultarTickets = async (params, callback) => {
        // Adiciona callback à fila de pendentes
        if (callback) {
          this.pendingCallbacks.push(callback);
        }

        // Se já há uma requisição em andamento, apenas adiciona o callback à fila
        if (this.consultarTicketsInProgress) {
          return;
        }
        ...
        // Cria novo timer para agrupar chamadas próximas
        this.consultarTicketsDebounceTimer = setTimeout(async () => {
          this.consultarTicketsInProgress = true;
          this.consultarTicketsDebounceTimer = null;

          try {
            const { data } = await ConsultarTickets(params);
            
            // Executa todos os callbacks pendentes com os dados
            this.pendingCallbacks.forEach(cb => {
              try {
                cb(data);
              } catch (err) {
                console.error('Erro ao executar callback de ConsultarTickets:', err);
              }
            });
          } catch (err) {
            // Silenciar erros de timeout - não mostrar no console
            if (err?.code !== 'ECONNABORTED' && !err?.message?.includes('timeout')) {
              console.log('error debouncedConsultarTickets', err);
            }
            
            // Executa callbacks mesmo em caso de erro (com undefined)
            this.pendingCallbacks.forEach(cb => {
              try {
                cb(undefined);
              } catch (e) {
                console.error('Erro ao executar callback de ConsultarTickets (erro):', e);
              }
            });
          } finally {
            // Limpa estado
        ...
        this.addTimer(this.consultarTicketsDebounceTimer);
      };
      ...
      socket.on(`${usuario.tenantId}:ticketList`, async (data) => {
        ...
        if (data.type === 'ticket:update') {
          ...
          debouncedConsultarTickets(params, (responseData) => {
            if (!responseData) return
            this.countTickets = responseData.count           
            // this.$store.commit('UPDATE_NOTIFICATIONS', responseData)
            if(usuario.profile === 'admin'){
              const openTickets = (responseData.tickets || []).filter(ticket => ticket.status === 'open' && ticket.unreadMessages > 0)
              const openCount = Number(openTickets.length) || 0
              this.$store.commit('UPDATE_NOTIFICATIONS', { ...responseData, tickets: openTickets, count: openCount })
            }
            else {
              const openTickets = responseData.tickets.filter(ticket => ticket.status === 'open' && ticket.userId === usuario.userId && ticket.unreadMessages > 0)
              const openCount = Number(openTickets?.length) || 0
              this.$store.commit('UPDATE_NOTIFICATIONS', { ...responseData, tickets: openTickets, count: openCount })
            }
            ...
            const newTickets = orderTickets(responseData.tickets)
            // NEWSOCKET
      ...
      socket.on(`${usuario.tenantId}:ticketList`, async data => {
        if (data.type === 'notification:new') {
          ...
          debouncedConsultarTickets(params, (data_noti) => {
            if (!data_noti) return
            ...
            if(usuario.profile === 'admin'){
              ...
              const pendingTickets = (data_noti.tickets || []).filter(ticket => ticket.status === 'pending' && ticket.userId === usuario.userId)
              const pendingCount = Number(pendingTickets.length) || 0
              if(pendingCount > 0){
                this.$store.commit('UPDATE_NOTIFICATIONS_P', { ...data_noti, tickets: pendingTickets, count: pendingCount })
              }
            ...
            var pass_noti = false
            if (data_noti?.tickets) {
              data_noti.tickets.forEach((element) => { pass_noti = (element.id == data.payload.id ? true : pass_noti) })
            }
            if (pass_noti) {
