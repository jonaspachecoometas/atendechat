...
const checkTicketFilter = (ticket) => {
  ...
  // Alteração: apenas "admin" tem acesso total (showAll), removendo "super"
  ...
  if(ticket.isGroup){
    if(mostrarGruposParaTodos === 'enabled'){
      ...
      if (isNotViewAssignedTickets() && (ticket?.userId || userId) !== userId &&
          (!userIdFiltro && !whatsappIdFiltro && !kanbanIdFiltro && !tagIdFiltro && !isQueuesTenantExists) &&
          (profile === 'admin' || (supervisorAdmin === 'disabled' && profile === 'super'))) {
        // console.log('isNotViewAssignedTickets e ticket não é do usuário', ticket?.userId, userId)
        // Alteração: permitir se profile for 'super' e ticket.status for 'open'
        if (profile === 'super' && ticket.status === 'open') {
          // Permitir visualizar tickets abertos atribuídos a usuários da equipe do supervisor
        } else {
      ...
      if (isQueuesTenantExists && (profile === 'user' || profile === 'super') && ticketsNulos !== 'enabled') {
