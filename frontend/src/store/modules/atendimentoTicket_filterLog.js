...
const atendimentoTicket = {
  ...
  mutations: {
    ...
    //     if (ticket.queue) {
    //       ticket.queue = normalizeQueue(ticket.queue)
    //     }
    ...
    LOAD_TICKETS (state, payload) {
      ...
      newTickets.forEach(ticket => {
        ...
        if (ticketIndex !== -1) {
          ...
          if (checkTicketFilter(ticket)) {
            ...
            // Rastrear distribuição por fila
            const queueName = ticket.queue || ticket.queueId || 'Sem fila'
            const queueKey = `${queueName} (ID: ${ticket.queueId || 'null'})`
            ticketsByQueue[queueKey] = (ticketsByQueue[queueKey] || 0) + 1
            
            // Rastrear distribuição por userId
            const userName = ticket.user?.name || 'Sem usuário'
            const userIdKey = `${userName} (ID: ${ticket.userId || 'null'})`
            ticketsByUserId[userIdKey] = (ticketsByUserId[userIdKey] || 0) + 1
            
            // Rastrear distribuição por whatsappId
            const whatsappName = ticket.whatsapp?.name || 'Sem WhatsApp'
            const whatsappIdKey = `${whatsappName} (ID: ${ticket.whatsappId || 'null'})`
            ticketsByWhatsappId[whatsappIdKey] = (ticketsByWhatsappId[whatsappIdKey] || 0) + 1
          } else {
            totalRejected++
          }
        }
      ...
      // Log da quantidade de tickets filtrados por fila (apenas se houver tickets filtrados)
      if (totalFiltered > 0) {
        console.log('=== Tickets Filtrados ===')
        console.log(`Total filtrados: ${totalFiltered}`)
        console.log(`Total rejeitados: ${totalRejected}`)
        console.log('================================')
        console.log('> Distribuição por fila:')
        Object.entries(ticketsByQueue).forEach(([queue, count]) => {
          console.log(`  ${queue}: ${count} ticket(s)`)
        })
        console.log('> Distribuição por usuário:')
        Object.entries(ticketsByUserId).forEach(([userId, count]) => {
          console.log(`  ${userId}: ${count} ticket(s)`)
        })
        console.log('> Distribuição por canal:')
        Object.entries(ticketsByWhatsappId).forEach(([whatsappId, count]) => {
          console.log(`  ${whatsappId}: ${count} ticket(s)`)
        })
        console.log('================================')
      }
    },
    ...
    UPDATE_TICKET (state, payload) {
      ...
      const ticketData = payload?.dataValues || payload
